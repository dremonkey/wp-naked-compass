...
Class Theme_Setup
{
	...
	function add_js(){
		...
		wp_enqueue_script( 'jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/1.6/jquery.min.js', '', false, true);
		
		// Do not load the scripts if on the admin page	
		if (!is_admin()){
...
Class Utilities 
{
	 ...
	 *
	 * @assumes all js files are in the theme_name/js directory
	 * @param path 
	...
	public function add_js($path, $req, $in_footer){
		...
		$name = Utilities::slice_string($path, ':-3');
		$name = Utilities::clean_string($path);
		$path = TEMPLATE_URL.$path;

		wp_enqueue_script( $name, $path, $req, false, $in_footer );
	}
