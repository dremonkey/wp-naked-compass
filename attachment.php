...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
				...
				<p><a href="<?php echo get_permalink( $post->post_parent ); ?>" title="<?php esc_attr( printf( __( 'Return to %s', THEME_NAME ), get_the_title( $post->post_parent ) ) ); ?>" rel="gallery"><?php
					/* translators: %s - title of parent post */
					printf( __( '<span>&larr;</span> %s', THEME_NAME ), get_the_title( $post->post_parent ) );
				?></a></p>

					<h2><?php the_title(); ?></h2>

						<?php
							printf(__('By %2$s', THEME_NAME),
						...
						<span>|</span>
						<?php
							printf( __('Published %2$s', THEME_NAME),
							...
							if ( wp_attachment_is_image() ) {
								...
								printf( __( 'Full size is %s pixels', THEME_NAME),
